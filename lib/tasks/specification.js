...
var ISSUE_PATTERN = /^(?:[-\w]+\/[-\w]+)?#\d+$/g;
var URL_PATTERN = /\bhttps?:\/\/(www\.)?[-a-zA-Z0-9@:%._\+~#=]{2,256}\.[a-z]{2,6}\b([-a-zA-Z0-9@:%_\+.~#?&//=]*)/ig;
var SpecificationTask = /** @class */ (function (_super) {
    __extends(SpecificationTask, _super);
    function SpecificationTask() {
        var _this = _super.call(this) || this;
        _this.name = "Specification";
        _this.description = "Checks for the presence of a issue tracker number in the pull request";
        _this.resolution = "Please check the rules of play docs [here](rulesofplay.com)";
        return _this;
    }
    SpecificationTask.prototype.run = function (context, config) {
                ...
                if (config.title) {
                    if (config.title && config.title["minimum-length"] && config.title["minimum-length"].enabled !== false) {
                        this.result.push({
                            ...
                            result: isLongEnough(pr.title, config.title["minimum-length"].length) ? "success" /* Success */ : "failure" /* Failure */
                        });
                    }
                }
                // Check body
                ...
                if (config.body) {
                    if (config.body && config.body["minimum-length"] && config.body["minimum-length"].enabled !== false) {
                        this.result.push({
                            ...
                            result: isLongEnough(pr.body, config.body["minimum-length"].length) ? "success" /* Success */ : "failure" /* Failure */
                        });
                    }
                    if (config.body["contains-issue-number"]) {
                        this.result.push({
                            label: "Pull Request body must contain issue number",
                            result: containsPattern(ISSUE_PATTERN, pr.body) ? "success" /* Success */ : "failure" /* Failure */
                        });
                    }
