...
var FourEyePrincipleTask = /** @class */ (function (_super) {
    ...
    function FourEyePrincipleTask(params) {
        ...
        _this.resolution = "Not enough people have approved this pull request - please ensure that XXX, who have not contributed to this pull request approve the changes.";
        _this.postAsComment = true;
        return _this;
    }
    FourEyePrincipleTask.prototype.run = function (context) {
        return __awaiter(this, void 0, void 0, function () {
            var author, directCollaborator, approvals, coAuthors, reviews, _i, _a, apr, missingApprovals;
            return __generator(this, function (_b) {
                switch (_b.label) {
                    case 0:
                        author = context.payload.pull_request.user.login;
                        directCollaborator = false;
                        if (!this.organization) return [3 /*break*/, 2];
                        return [4 /*yield*/, this.getOrgMembershipStatus(this.organization, author, context)];
                    case 1:
                        directCollaborator = _b.sent();
                        return [3 /*break*/, 4];
                    case 2: return [4 /*yield*/, this.getRepoMembershipStatus(this.repo.owner, this.repo.repo, author, context)];
                    case 3:
                        directCollaborator = _b.sent();
                        _b.label = 4;
                    case 4:
                    ...
                    case 6:
                        ...
                        for (_i = 0, _a = reviews.approvals; _i < _a.length; _i++) {
                            apr = _a[_i];
                            this.result.push({
                        ...
                        if (reviews.contributing.length > 0) {
                            this.result.push({
                                label: plural_1.plural('Approvals', 'Approval', reviews.contributing.length) + " from " + reviews.contributing.map(function (x) { return "@" + x; }).join(", ") + " was excluded as the pull request contains changes from " + plural_1.plural('those users', 'this user', reviews.contributing.length),
                                result: "warning" /* Warning */
                            });
                        }
                        if (approvals < this.config.minimum) {
                            missingApprovals = this.config.minimum - approvals;
                            this.result.push({
                                label: missingApprovals + " additional " + plural_1.plural('approvals', 'approval', missingApprovals) + " needed",
                                result: "failure" /* Failure */
                            });
                            this.resolution = this.resolution.replace('XXX', plural_1.plural(missingApprovals.toString() + " additional users", "1 additional user", missingApprovals));
                        }
            ...
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        ...
                        return [4 /*yield*/, context.github.repos.compareCommits(__assign({}, this.repo, { base: pr.base.sha, head: pr.head.sha }))];
                    case 1:
                        response = _a.sent();
                        /*
                        ...
                        return [2 /*return*/, response.data.commits
                                .filter(function (data) { return data !== null && data.author !== null && data.author.login; })
                                .filter(function (data) { return data.commit.message.indexOf("Co-Authored-By: " + pr_author) < 0; })
                                .filter(function (data) { return (data.commit.committer.login !== 'Github' && data.commit.message.indexOf("Merge branch '" + pr.base.ref + "' into") < 0); })
                                .map(function (data) { return data.author.login; })
                                .filter(this.unique)];
                }
            });
        });
    ...
    FourEyePrincipleTask.prototype.getRepoMembershipStatus = function (owner, repo, login, context) {
        return __awaiter(this, void 0, void 0, function () {
            var response, permission;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0: return [4 /*yield*/, context.github.repos.getCollaboratorPermissionLevel({ owner: owner, repo: repo, username: login })];
                    case 1:
                        response = _a.sent();
                        permission = response.data.permission;
                        return [2 /*return*/, (permission === "write" || permission === "admin")];
                }
            });
        });
    };
