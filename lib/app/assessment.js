...
function postAssessment(req, res) {
    return __awaiter(this, void 0, void 0, function () {
        var octokit, gh_repo, branched_riskFile, result, err_1;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    if (!(req.session && ghe_host)) return [3 /*break*/, 7];
                    _a.label = 1;
                case 1:
                    _a.trys.push([1, 5, , 6]);
                ...
                case 2:
                    ...
                    if (!branched_riskFile) return [3 /*break*/, 4];
                    return [4 /*yield*/, updateRiskFile(req.body, branched_riskFile.sha, octokit, gh_repo, rr_branch)];
                case 3:
                    result = _a.sent();
                    if (!result) {
                        res.status(500);
                ...
                case 4:
                    // ensure PR is open
                    // update descripting with stats
                    /* if branch already exists
                    if(prs.data.length > 0){
                      const riskFile = await octokit.repos.getContents({ path: "risks.md", ...gh_repo, ref: rr_branch });
                      const sha = riskFile.data.sha;
                      const content = Buffer.from(YAML.stringify(req.body)).toString('base64');
                      await octokit.repos.updateFile({...gh_repo, path: "risks.md", sha: sha, content: content, message: "updated risk file", branch: "rapid-risk"});
                    }else{
